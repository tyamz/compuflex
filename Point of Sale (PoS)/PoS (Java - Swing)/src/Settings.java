...
public class Settings {
	...
	void loadProperties() {
		try {
			...
			in = new FileInputStream("Compuflex_PoS_Swing_2.1.properties");
			...
			this.setProductFile(new File("products/" + this.applicationProps.getProperty("productFile")));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void saveProperties() throws IOException {
		// save properties
		FileOutputStream out = new FileOutputStream("Compuflex_PoS_Swing_2.1.properties");
		this.applicationProps.setProperty("popUp", popUp.toString());
		this.applicationProps.setProperty("userDisplay", userDisplayOption + "");
		this.applicationProps.setProperty("productFile", this.getProductFile().getName());
