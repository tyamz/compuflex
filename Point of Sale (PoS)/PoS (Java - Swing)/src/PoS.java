...
public class PoS {
	...
	private void initialize() {		
		File f = new File("Compuflex_PoS_Swing_2.1.properties");
		
		if(f.isFile()) {
			u = new User("Default User");
			appSettings = new Settings();
			cart = new Cart(appSettings);
			trans = new TransactionRecord();
			fmt = cart.getFmt();
			dateFmt = trans.getDateFmt();
			try {
				printer = new PrintWriter(new File("Compuflex_PoS_Swing_2.1.log"));
			...
			name = TextToGraphics.convertToImage(u.getName());
			imagePanel = new JPanel();
			menuBar.add(imagePanel);
			imageLabel = new JLabel(new ImageIcon(name));
		...
		if(!f.isFile()) {
			...
			if(!d.isFile()) {
				try {
					...
					printDef.println("# Compuflex - Point of Sale (PoS) - Swing Version 2.1\r\n" + 
	...
	private void setUser(String name) {
		...
		this.name = TextToGraphics.convertToImage(u.getName());
		imageLabel.setIcon(new ImageIcon(this.name));
	}
